    ...
    <article className="group overflow-hidden rounded-xl border border-border bg-card shadow-card hover:shadow-elevated hover:border-primary/30 transition">
      <Link to="/property/$id" params={{ id: p.id }} className="block relative aspect-[16/10] overflow-hidden bg-muted">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          <HiveVerifiedBadge />
          {p.featured && <span className="rounded-md bg-accent text-accent-foreground text-[10px] font-bold px-2 py-1 shadow-sm">FEATURED</span>}
          {p.premium && <span className="rounded-md bg-primary text-primary-foreground text-[10px] font-bold px-2 py-1 shadow-sm">PREMIUM</span>}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground">
          {p.category === "home" && p.bhk && (
            <span className="inline-flex items-center gap-1"><Icon d="M3 21V8l9-5 9 5v13M9 21v-6h6v6" /> {p.bhk} BHK</span>
          )}
          ...
          {p.category === "home" && p.bathrooms && (
            <span className="inline-flex items-center gap-1"><Icon d="M4 12h16v4a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4v-4z M6 12V6a2 2 0 0 1 4 0" /> {p.bathrooms} Bath</span>
          )}
          {p.category === "land" && p.land && (
            <span className="inline-flex items-center gap-1 text-primary/80 font-medium">{p.land.naStatus}</span>
          )}
          <span className="inline-flex items-center gap-1 text-primary/80">{p.subType}</span>
        </div>

        <div className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-3">
          <a
            href={telHref}
            className="inline-flex items-center justify-center gap-1.5 rounded-md bg-secondary text-primary hover:bg-primary hover:text-primary-foreground py-2 text-xs font-semibold transition"
          >
...
export function HiveVerifiedBadge({ large = false }: { large?: boolean }) {
  return (
    ...
    >
      <svg width={large ? 14 : 11} height={large ? 14 : 11} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
        ...
        <path d="M9.5 12.5l1.75 1.75L15 10.5" stroke="var(--success)" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
      Hive Verified
    </span>
  );
