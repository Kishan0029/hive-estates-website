...
export function SearchBar({ defaultMode = "apartments" }: { defaultMode?: Mode }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl bg-background border border-border shadow-elevated p-4 md:p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-2">
        <label className="rounded-lg border border-border bg-card px-4 py-3 flex items-center gap-3 focus-within:border-primary">
          ...
          <input
            ...
            placeholder={mode === "land" ? "Search land by locality, area, tags or keywords…" : "Search homes by locality, area, tags or keywords…"}
            className="w-full bg-transparent outline-none text-sm"
          />
          <datalist id="locality-suggestions">{LOCALITIES.map((l) => <option key={l} value={l} />)}</datalist>
        </label>
        <button type="submit" className="rounded-lg bg-primary text-primary-foreground font-semibold px-8 py-3 hover:opacity-90 transition">
          Search
        </button>
      </div>
    </form>
